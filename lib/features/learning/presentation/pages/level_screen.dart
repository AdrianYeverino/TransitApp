          ...
          return StreamBuilder<DocumentSnapshot>(
            ...
            builder: (context, snapshotUser) {
              ...
              List<dynamic> desbloqueados =
                  userData['subniveles_desbloqueados'] ?? ['basico_s1'];
              ...
              bool _isSubnivelDesbloqueado(int index, String tagSubnivel) {
                ...
                // 2. Si está en la lista de desbloqueados
                if (desbloqueados.contains(tagSubnivel)) return true;

                // 3. Si es el primer nivel de basico, siempre desbloqueado
                if (levelId == 'basico' && index == 0) return true;

                // 4. Si todos los niveles anteriores están completados, este se desbloquea
                bool todosAnterioresCompletados = true;
                for (int i = 0; i < index; i++) {
                  String anteriorTag = '${levelId}_${sublevels[i].id}';
                  if (!completados.contains(anteriorTag)) {
                    todosAnterioresCompletados = false;
                    break;
                  }
                }
                if (todosAnterioresCompletados) return true;

                // 5. Si el nivel anterior está completado, este se desbloquea
                if (index > 0) {
                  String anteriorTag = '${levelId}_${sublevels[index - 1].id}';
                  if (completados.contains(anteriorTag)) return true;
                }
                  ...
                  children: [
                    ...
                    ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        var subData =
                            sublevels[index].data() as Map<String, dynamic>;
                        String subId = sublevels[index].id;
                        String tagSubnivel = '${levelId}_$subId';
                        String nextId = (index == sublevels.length - 1)
                            ? 'examen'
                    ...
                    _buildBotonLeccion(
                      ...
                      levelId: levelId,
                      subId: 'examen',
                      titulo: 'Examen Final de Módulo',
                      desc: 'Demuestra todo lo que aprendiste.',
                      xp: '200 XP',
                      desbloqueada: examenDesbloqueado,
                      completada: examenCompletado,
                      isFinalExam: true,
                      nextSubId:
                          's1', // El siguiente de un examen es el s1 del mundo que sigue
                    ),
                    const SizedBox(height: 30),
                  ],
                ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(iconoNivel, color: Colors.white, size: 50),
            ...
            ? () => Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => QuizPage(
                    ...
                    subLevelId: subId,
                    title: titulo,
                    xpRecompensa: int.parse(xp.split(' ')[0]),
                    isFinalExam: isFinalExam,
                    nextSubLevelId: nextSubId,
                  ),
                ),
              )
            : () {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Comienza con las lecciones anteriores.'),
                  ),
                );
              },
        child: ClipRRect(
          ...
          child: Stack(
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: isFinalExam ? Colors.amber.shade50 : Colors.white,
                  border: Border.all(
                    color: completada
                  ...
                  children: [
                    CircleAvatar(
                      backgroundColor: completada
                          ? Colors.green
                          : (desbloqueada
                                ? (isFinalExam ? Colors.amber : colorNivel)
