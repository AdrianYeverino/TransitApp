...
class _HomePageState extends State<HomePage> {
            ...
            children: [
              ...
              StreamBuilder<DocumentSnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  var userData = snapshot.data!.data() as Map<String, dynamic>?;
                  List<dynamic> desbloqueados = userData?['subniveles_desbloqueados'] ?? [];
                  
                  bool intermedioLibre = desbloqueados.contains('intermedio_s1');
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () {
                          Navigator.push(context, MaterialPageRoute(builder: (context) => const LevelScreen(
                            levelId: 'basico', tituloNivel: 'Nivel Básico', colorNivel: Colors.blue, iconoNivel: Icons.shield,
                          )));
                        },
                        child: _buildNivelCard(
                          ...
                          icono: Icons.shield,
                          titulo: 'Nivel Básico',
                          descripcion: 'Fundamentos de vialidad y seguridad para todos.',
                      ...
                      GestureDetector(
                        onTap: intermedioLibre ? () {
                          Navigator.push(context, MaterialPageRoute(builder: (context) => const LevelScreen(
                            levelId: 'intermedio', tituloNivel: 'Nivel Intermedio', colorNivel: Colors.orange, iconoNivel: Icons.traffic,
                          )));
                        } : () {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Debes completar el Nivel Básico y su examen final primero.')));
                        },
                        child: _buildNivelCard(
                          color: Colors.orange.shade600,
                          icono: Icons.traffic,
                          titulo: 'Nivel Intermedio',
                          descripcion: 'Señalización restrictiva, reglas de tránsito y multas.',
                          progresoText: intermedioLibre ? 'Toque para entrar' : 'Bloqueado',
                      ...
                      GestureDetector(
                        onTap: avanzadoLibre ? () {
                          Navigator.push(context, MaterialPageRoute(builder: (context) => const LevelScreen(
                            levelId: 'avanzado', tituloNivel: 'Nivel Avanzado', colorNivel: Colors.redAccent, iconoNivel: Icons.local_hospital,
                          )));
                        } : () {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Debes completar el Nivel Intermedio primero.')));
                        },
                        child: _buildNivelCard(
                          color: Colors.redAccent.shade400,
                          icono: Icons.local_hospital,
                          titulo: 'Nivel Avanzado',
                          descripcion: 'Protocolos de emergencia, siniestros y primeros auxilios.',
                          progresoText: avanzadoLibre ? 'Toque para entrar' : 'Bloqueado',
  ...
  Widget _buildMotivationCard() {
    return Container(
      ...
      padding: const EdgeInsets.all(16.0),
      child: const Text('¡Sigue practicando para desbloquear nuevos mundos!', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
    );
  }
}
